...
layout(location = 0) in float3 _vsin_In_f3PosWS;
layout(location = 1) in float2 _vsin_In_f2UV;
layout(location = 2) in float3 _vsin_In_SubStruct_f3Normal;
layout(location = 3) in float3 _vsin_f3UV;
layout(location = 0) out float4 _vsout_TestVS;
layout(location = 1) out float2 _vsout_Out_f2Attrib;
layout(location = 2) out float3 _vsout_Out_f3Attrib;
layout(location = 3) out float4 _vsout_Out_f4Attrib;
layout(location = 4) out flat uint _vsout_Out_uiAttrib;
layout(location = 5) out flat uint3 _vsout_Out_ui3Attrib;
layout(location = 6) out float4 _vsout_Out_SubStruct_f4Attrib;
layout(location = 7) out flat int _vsout_Out_SubStruct_iAttrib;
layout(location = 8) out float _vsout_fAttrib;
layout(location = 9) out flat int4 _vsout_i4Attrib;

#define _RETURN_(_RET_VAL_){\
_vsout_TestVS = _RET_VAL_;\
_vsout_Out_f2Attrib = Out.f2Attrib;\
_vsout_Out_f3Attrib = Out.f3Attrib;\
_vsout_Out_f4Attrib = Out.f4Attrib;\
_vsout_Out_uiAttrib = Out.uiAttrib;\
_vsout_Out_ui3Attrib = Out.ui3Attrib;\
_SET_GL_POSITION(Out.f4PosPS);\
_vsout_Out_SubStruct_f4Attrib = Out.SubStruct.f4Attrib;\
_vsout_Out_SubStruct_iAttrib = Out.SubStruct.iAttrib;\
_vsout_fAttrib = fAttrib;\
_vsout_i4Attrib = i4Attrib;\
return;}

void main  ()
{
    VSInput In;
    In.f3PosWS = _vsin_In_f3PosWS;
    In.f2UV = _vsin_In_f2UV;
    In.SubStruct.f3Normal = _vsin_In_SubStruct_f3Normal;
    _GET_GL_VERTEX_ID(In.SubStruct.VertexId);
    float3 f3UV;
    f3UV = _vsin_f3UV;
    uint InstID;
    _GET_GL_INSTANCE_ID(InstID);
    VSOutput Out;
    float fAttrib;
    int4 i4Attrib;
    ...
    if( In.f2UV.x < 0.5 )
        _RETURN_( float4(0.2,0.3,0.4,0.5))
    ...
    _RETURN_( float4(0.5,0.6,0.7,0.8))
}
