...
layout(std140, binding = 1) uniform BallBuffer
{
	float3 LightPos;
...
float3 GetColor(PS_INPUT input, float3 inNormal)
{
	...
	else
	{
		...
		if (d < textureSquareSize)
		{
			...
			float3 texColor = Sample_2( TexDiffuse,TexDiffuse_sampler, tex)_SWIZZLE0.xyz;
